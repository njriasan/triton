// RUN: triton-opt %s -split-input-file -convert-triton-to-tritongpu='target=cuda:sm80 num-warps=2' | FileCheck %s
// RUN: triton-opt %s -split-input-file -convert-triton-to-tritongpu='target=cuda:sm80 num-warps=2 num-ctas=2' | FileCheck %s --check-prefixes=CHECK-TWO-CTAS

module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 2 : i32} {
tt.func @ops() {
  // CHECK: module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 2 : i32, ttg.target = "cuda:sm80", "ttg.threads-per-warp" = 32 : i32} {{.*}}
